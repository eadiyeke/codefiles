function[P1,P2,AVG,CTG,ffinalAVG,ffinalCAT]= forestprediction3oobe(forest,ntree,newinstance,x,y,x_eval,y_eval,ts)
...
%  end
% end 
us=zeros(1,ntree);%since we accept all predictions of the trees
...
%         end
%       end
       [y1_avg, y2_type]=sendleaf5(forest{k,1},newinstance,x,y,x_eval,y_eval);
       P1{1,k}=y1_avg;
       P2{1,k}=y2_type;
   end
    finalavgs={};
...
%finalavgs2{1,1}=s;
 ...
 for k2=1:size(finalavgs2_,2)
     ...
     [c cindex]=max(z2);
        z11=z1(1,1:z3);
        grr=finavv(z11,1);
        CTG_=grr;%if u have equal votes pick the first in lexic order
   CTG(k2)=CTG_(1,1);
%         [c cindex]=max(z2);
%    CTG_=cindex-1;
%    CTG(k2)=CTG_;
